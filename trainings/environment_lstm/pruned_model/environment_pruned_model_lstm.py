"""
Script: environment_pruned_model_lstm.py
Module role:
    Train, prune, fine-tune, and evaluate the pruned LSTM model adopted in the
...
# --- bootstrap: Allows importing utilities/ locally and in the runner ---
...
# -----------------------------------------------------------------
from utils.global_utils.paths_lstm import PROJECT_ROOT, DATASET_ENVIRONMENT_LSTM, BASE_MODEL, PRUNED_MODEL, PRUNED_MODEL_METRICS # use project root in a stable way
...
# Paths
# ---------------------
model_path = version_path/"environment_base_model_lstm.keras"
scaler_X_path = version_path/"environment_base_model_lstm_scaler_X.pkl"
scaler_y_path = version_path/"environment_base_model_lstm_scaler_y.pkl"

# =========================
# Load data and features (same residual pipeline + 24-step window as the baseline)
# =========================
dataset_path = DATASET_ENVIRONMENT_LSTM / "environment_dataset_lstm.csv"
df = pd.read_csv(dataset_path)

# Check expected columns (without absolute humidity)
...
# Internal residual target: ΔT_in, ΔH_in (same as base)
y_all = np.stack([
...
SEQ_LEN = 24   # temporal window (for example, 24 steps)
...
for t in range(SEQ_LEN - 1, N_total):
    X_seq.append(X_source[t - SEQ_LEN + 1 : t + 1, :])  # (seq_len, 12)
    y_seq.append(y_all[t])                              # (2,)
    idx_seq.append(t)

X_seq = np.asarray(X_seq, dtype=np.float32)  # (N, SEQ_LEN, 12)
y_seq = np.asarray(y_seq, dtype=np.float32)  # (N, 2)
idx_seq = np.asarray(idx_seq, dtype=np.int64)
N_seq, _, K_NUM_FEATURES = X_seq.shape  # K_NUM_FEATURES must be 12

# ---------------------
# Temporal 60/20/20 split in sequences (same as the baseline residual pipeline)
# ---------------------
i1, i2 = int(0.6 * N_seq), int(0.8 * N_seq)
X_train, X_val, X_test = X_seq[:i1], X_seq[i1:i2], X_seq[i2:]
y_train, y_val, y_test = y_seq[:i1], y_seq[i1:i2], y_seq[i2:]
...
# ---------------------
# Load scalers from the baseline model (already trained with 12 features + residual target)
# ---------------------
scaler_X = joblib.load(scaler_X_path)
scaler_y = joblib.load(scaler_y_path)

# Normalize X using the baseline scaler (do not refit)
F = K_NUM_FEATURES

N_train = X_train.shape[0]
X_train_2d = X_train.reshape(-1, F)
X_train_2d = np.clip(X_train_2d, scaler_X.data_min_, scaler_X.data_max_).astype(np.float32)
X_train = scaler_X.transform(X_train_2d).reshape(N_train, SEQ_LEN, F)
N_val = X_val.shape[0]
X_val_2d = X_val.reshape(-1, F)
X_val_2d = np.clip(X_val_2d, scaler_X.data_min_, scaler_X.data_max_).astype(np.float32)
X_val = scaler_X.transform(X_val_2d).reshape(N_val, SEQ_LEN, F)
N_test = X_test.shape[0]
X_test_2d = X_test.reshape(-1, F)
X_test_2d = np.clip(X_test_2d, scaler_X.data_min_, scaler_X.data_max_).astype(np.float32)
X_test = scaler_X.transform(X_test_2d).reshape(N_test, SEQ_LEN, F)
# Normalize y (residual ΔT_in, ΔH_in) with the base scaler
y_train = scaler_y.transform(y_train)
y_val   = scaler_y.transform(y_val)
y_test  = scaler_y.transform(y_test)

# ---------------------
# Load the baseline model (already trained on the residual target)
# ---------------------
base_model = tf.keras.models.load_model(model_path)

# ---------------------
# Define the dynamic pruning schedule
# ---------------------
batch_size = 64
epochs_prune = 20  # can be adjusted
steps_per_epoch = int(np.ceil(len(X_train) / batch_size))
end_step = steps_per_epoch * epochs_prune

pruning_schedule = sparsity.PolynomialDecay(
    initial_sparsity=0.2,
    final_sparsity=0.5,   # adjust if accuracy degrades
    begin_step= steps_per_epoch * 10,
    end_step= end_step
)
...
# ---------------------
# Compile with a reduced learning rate for stability
# ---------------------
...
# ---------------------
# Remove the wrapper and save the pruned model
# ---------------------
final_model = sparsity.strip_pruning(pruned_model)
final_model.save(run_dir/"environment_pruned_model_lstm.keras")

# ---------------------
# Function for real sparsity
# ---------------------
def calc_sparsity(m):
    zeros = 0
    total = 0
...
_ = final_model.predict(X_test, verbose=0)
end_time = time.time()
inference_time_total = (end_time - start_time) * 1000  # em milissegundos
...
# Model size in bytes
# ---------------------
saved_model_path = run_dir/"environment_pruned_model_lstm.keras"
...
# ---------------------
# Normalized evaluation (residual target space)
# ---------------------
...
# ---------------------
# Evaluation in original absolute scale (T_in, H_in)
# ---------------------
# y_pred_test_scaled → normalized residuals; inverse-transform and add to the lags
...
# Ensure that previous values (lag1) are float32
...
# ============================================================
# GATING IGUAL AO FIRMWARE:
# - invoked_mask: houve Invoke real
# - is_rollover_mask: EVENT type=HOUR (TRIG does not enter the metrics)
# In offline test mode, all entries are True by default.
# ============================================================
...
def _r2_like_firmware(y_true_1d: np.ndarray, y_pred_1d: np.ndarray) -> float:
    ...
    Replicate the firmware logic (metrics.cpp):
      ...
      - Se n < 2: R² indefinido (NaN)
      - If SS_tot <= 1e-6: very low variance → R² undefined (NaN)
    """
...
def _metrics_like_firmware(y_true_2d: np.ndarray, y_pred_2d: np.ndarray):
    ...
    mae_w = 0.5 * (mae_T + mae_H)
    rmse_w = float(np.sqrt(0.5 * (mse_Tw + mse_Hw)))
    r2_w = 0.5 * (r2_Tw + r2_Hw)
    return mae_w, rmse_w, r2_w, mae_T, rmse_Tw, r2_Tw, mae_H, rmse_Hw, r2_Hw
...
if n_test < ROLLING_N:
    ...
    for end in range(ROLLING_N - 1, n_test):
        ...
        # If there was no real Invoke or this is not a rollover (HOUR), do not update metrics.
...
# Useful summaries for comparison with the firmware
    ...
    df_roll.to_csv(metrics_run_dir / "environment_pruned_model_lstm_metrics_rolling24.csv",
                   index=False, encoding="utf-8-sig")
    excel_roll_path = metrics_run_dir / "environment_pruned_model_lstm_metrics_rolling24.xlsx"
    ...
    print("[INFO] Rolling files saved:")
    print(" - environment_pruned_model_lstm_metrics_rolling24.csv")
    print(" - environment_pruned_model_lstm_metrics_rolling24.xlsx")
    ...
    plt.title("Rolling-Window Performance (N=24) — Firmware-Comparable Pruned Model")
    plt.xlabel("Time (window end)")
    plt.ylabel("Valor")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(metrics_run_dir / "environment_pruned_model_lstm_metrics_rolling24_mae_rmse.png")
    plt.close()

    # Plot: rolling R²
    plt.figure(figsize=(10, 4))
    plt.plot(df_roll["datetime_end"], df_roll["R2"], label="R² (rolling24)")
    plt.title("Rolling-Window Coefficient of Determination (N=24) — Firmware-Comparable Pruned Model")
    plt.xlabel("Time (window end)")
    plt.ylabel("R²")
    plt.ylim([-0.2, 1.05])
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(metrics_run_dir / "environment_pruned_model_lstm_metrics_rolling24_r2.png")
    plt.close()
...
# Combined diagnosis based on absolute gap, percentage gap, and absolute losses
...
plt.title("Training Convergence During Pruning\n" + status)
plt.xlabel("Epochs")
plt.ylabel("Loss (MSE)")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(metrics_run_dir/"environment_training_validation_loss_diagnosis_pruning_lstm.png")
plt.close()
...
plt.title("Metrics Summary — Pruned Model")
plt.grid(axis='y')
for bar in bars:
    yval = bar.get_height()
    plt.text(bar.get_x() + bar.get_width() / 2.0, yval, f"{yval:.4f}", ha='center', va='bottom')
plt.tight_layout()
plt.savefig(metrics_run_dir/"environment_final_metrics_summary_plot_pruned_lstm.png")
plt.close()

# Joint scatter plot (T_in + H_in)
plt.figure(figsize=(6, 6))
plt.scatter(y_test_orig[:, 0], y_pred_orig[:, 0], alpha=0.5, label="Temperature (T_in)", color="blue")
plt.scatter(y_test_orig[:, 1], y_pred_orig[:, 1], alpha=0.5, label="Humidity (H_in)", color="green")
...
plt.grid(True)
plt.tight_layout()
plt.savefig(metrics_run_dir/"environment_scatter_predictions_pruned_lstm.png")
plt.close()
...
plt.title("Scatter Plot — Temperature (T_in) - Pruned Model")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(metrics_run_dir/"environment_pruned_model_lstm_scatter_T_in.png")
plt.close()
...
plt.xlabel("Actual H_in (%)")
plt.ylabel("Predicted H_in (%)")
plt.title("Scatter Plot — Humidity (H_in) - Pruned Model")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(metrics_run_dir/"environment_pruned_model_lstm_scatter_H_in.png")
plt.close()
...
metrics_dist = {
    ...
    "Valor": [
        ...
        f"{sparsity_before * 100:.2f} %", f"{sparsity_after * 100:.2f} %", f"{tamanho_kb:.2f} KB",
...
df_tbl = pd.DataFrame(metrics_dist)

# Save as CSV
df_tbl.to_csv(metrics_run_dir/"environment_pruned_model_lstm_metrics_summary.csv", index=False, encoding="utf-8-sig")
print("[INFO] File saved: environment_pruned_model_lstm_metrics_summary.csv")

# Save as Excel with automatic column-width adjustment
excel_path = metrics_run_dir/"environment_pruned_model_lstm_metrics_summary.xlsx"
df_tbl.to_excel(excel_path, index=False)
...
wb.save(excel_path)
print("[INFO] File saved: environment_pruned_model_lstm__metrics_summary.xlsx (with adjusted columns)")
