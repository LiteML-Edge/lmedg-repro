...
epochs_axis = np.arange(1, len(train_loss_hist) + 1)

plt.figure(figsize=(8, 5))
plt.plot(epochs_axis, train_loss_hist, label="Training")
plt.plot(epochs_axis, val_loss_hist, label="Validation")

plt.title(
    "Quantized Conv1D Tiny: loss during fine-tuning — "f"training-validation difference = {gap_pct:.2f}%"
)
plt.xlabel("Epoch")
...
_savefig_pub("environment_Conv1D_Tiny_training_validation_loss_epoch")
plt.close()
...
plt.title("Quantized Conv1D Tiny: indoor temperature — ground truth vs prediction")
plt.grid(True)
plt.tight_layout()
...
plt.ylabel("Prediction H_in (%)")
plt.title("Quantized Conv1D Tiny: indoor humidity — ground truth vs prediction")
plt.grid(True)
plt.tight_layout()
...
if len(accepted_idxs) >= ROLLING_N:
    ...
    try:
        ...
        plt.xlabel("Ground-truth H_in (%)")
        plt.ylabel("Prediction H_in (%)")
        plt.title("Quantized LSTM: indoor humidity — ground truth vs prediction")
        plt.grid(True, linestyle="--", alpha=0.5)
        plt.tight_layout()
        ...
        try:
            ...
            if x_dt.isna().all():
                ...
                plt.xlabel("Rolling-24 window end")
        except Exception:
            x_dt = np.arange(len(df_roll_24))
            plt.xlabel("Rolling window index (last 24)")

        plt.plot(x_dt, yT_true_r, label="Ground-truth T_in (°C)")
        plt.plot(x_dt, yT_pred_r, label="Prediction T_in (°C)")
        ax = plt.gca()
        ax.xaxis.set_major_formatter(mdates.DateFormatter("%d/%m\n%Hh"))
        plt.ylabel("T_in temperature (°C)")
        plt.title("Quantized Conv1D Tiny: Rolling-24 for indoor temperature")
        plt.grid(True, linestyle="--", alpha=0.5)
        plt.legend()
        plt.tight_layout()
        ...
        ax = plt.gca()
        ax.xaxis.set_major_formatter(mdates.DateFormatter("%d/%m\n%Hh"))
        plt.ylabel("H_in relative humidity (%)")
        plt.title("Quantized Conv1D Tiny: Rolling-24 for indoor humidity")
        plt.grid(True, linestyle="--", alpha=0.5)
        plt.legend()
        plt.tight_layout()
