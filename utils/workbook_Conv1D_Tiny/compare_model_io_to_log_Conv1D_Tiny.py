"""
Script: compare_model_io_to_log_Conv1D_Tiny.py
Module role:
...
from utils.global_utils.paths_Conv1D_Tiny import (  # type: ignore
    PROJECT_ROOT,
...
def resolve_runtime_paths(
    ...
    input_ref_path = find_reference_xlsx(
        ...
        exact_name="environment_quantized_dbg_model_input_reference_Conv1D_Tiny.xlsx",
        fallback_patterns=[
            "*dbg_model_input_reference*Conv1D_Tiny*.xlsx",
            "*input_reference*Conv1D_Tiny*.xlsx",
            "*input*reference*Conv1D_Tiny*.xlsx",
            "*dbg*input*Conv1D_Tiny*.xlsx",
        ],
    )

    output_ref_path = find_reference_xlsx(
        results_dir=results_dir,
        exact_name="environment_quantized_dbg_model_output_reference_Conv1D_Tiny.xlsx",
        fallback_patterns=[
            "*dbg_model_output_reference*Conv1D_Tiny*.xlsx",
            "*output_reference*Conv1D_Tiny*.xlsx",
            "*output*reference*Conv1D_Tiny*.xlsx",
            "*dbg*output*Conv1D_Tiny*.xlsx",
        ],
    )
    ...
    try:
        output_raw_ref_path: Optional[Path] = find_reference_xlsx(
            results_dir=results_dir,
            exact_name="environment_quantized_dbg_model_output_raw_reference_Conv1D_Tiny.xlsx",
            fallback_patterns=[
                "*dbg_model_output_raw_reference*Conv1D_Tiny*.xlsx",
                "*output_raw_reference*Conv1D_Tiny*.xlsx",
                "*output*raw*reference*Conv1D_Tiny*.xlsx",
                "*dbg*output*raw*Conv1D_Tiny*.xlsx",
            ],
        )
    ...
    try:
        preprocess_ref_path: Optional[Path] = find_reference_xlsx(
            results_dir=results_dir,
            exact_name="environment_quantized_preprocess_debug_reference_Conv1D_Tiny.xlsx",
            fallback_patterns=[
                "*preprocess_debug_reference*Conv1D_Tiny*.xlsx",
                "*preprocess*debug*reference*Conv1D_Tiny*.xlsx",
                "*preprocess*reference*Conv1D_Tiny*.xlsx",
            ],
        )
    except FileNotFoundError:
        preprocess_ref_path = None

    workbook_root = Path(PROJECT_ROOT) / "utils" / "workbook_Conv1D_Tiny"
    # Search recursively below the Conv1D_Tiny firmware root so logs from both
    ...
    if cli_out:
        ...
        preferred = (
            ...
            / platform_slug
            / mode_slug
            / f"model_io_comparison_Conv1D_Tiny_{platform_slug}_{mode_slug}.xlsx"
        )
...
def build_human_summary(
    ...
    if input_status != "MATCH":
        conclusion = "The critical model input tensor still diverges; fix input packaging before interpreting any Conv1D_Tiny output mismatch."
        interpretation = (
...
def add_executive_summary_sheet(
    ...
    ws["A2"] = f"MODEL: Conv1D_Tiny | PLATFORM: {platform_label} | MODE: {mode_slug.upper()}"
