...
def resolve_runtime_paths(
    ...
    if cli_pred:
        ...
        if not QUANTIZED_MODEL_METRICS:
            raise FileNotFoundError(
                "Prediction spreadsheet path not provided and utils.paths_Conv1D_Tiny is unavailable."
            )
    ...
    if cli_metrics:
        ...
        if results_dir is None:
            if not QUANTIZED_MODEL_METRICS:
                raise FileNotFoundError(
                    "Metrics spreadsheet path not provided and utils.paths_Conv1D_Tiny is unavailable."
                )
            metrics_root = normalize_metrics_root(Path(QUANTIZED_MODEL_METRICS))
            latest_run_dir = find_latest_run_dir(metrics_root)
            results_dir = latest_run_dir / "quantization_metrics_results"
    ...
    if cli_log:
        ...
        if cli_log_dir:
            ...
            if not PROJECT_ROOT:
                raise FileNotFoundError(
                    "Log path/directory not provided and utils.paths_Conv1D_Tiny is unavailable."
                )
    ...
    if cli_out:
        ...
        if cli_out_dir:
            ...
            if not PROJECT_ROOT:
                raise FileNotFoundError(
                    "Output path/directory not provided and utils.paths_Conv1D_Tiny is unavailable."
                )
        ...
        preferred = (
            ...
            / platform_slug
            / mode_slug
            / f"predictions_metrics_vs_log_comparison_Conv1D_Tiny_{platform_slug}_{mode_slug}.xlsx"
        )
