# MLP audit workbooks by platform

This package contains two scripts for validating **Replay** logs from the MLP firmware and generating separate workbooks for each hardware platform:

- `compare_model_io_to_log_mlp.py`
- `compare_predictions_metrics_to_log_mlp.py`
...
- `pandas`;
- `openpyxl`;
- project structure containing `utils/global_utils/paths_mlp.py`.
...
Default structure used when paths are not provided through arguments. The scripts search recursively under `firmwares/environment_mlp` to locate compatible logs:
...
```bash
python compare_model_io_to_log_mlp.py --platform all --mode replay
```

Predictions and metrics:

```bash
python compare_predictions_metrics_to_log_mlp.py --platform all --mode replay
```

### Use explicit directories

```bash
python compare_model_io_to_log_mlp.py \
  --platform all \
  --mode replay \
  --log-dir firmwares/environment_mlp/PlatfIO_mlp/logs \
  --out-dir utils/workbook_mlp
```

```bash
python compare_predictions_metrics_to_log_mlp.py \
  --platform all \
  --mode replay \
  --log-dir firmwares/environment_mlp/PlatfIO_mlp/logs \
  --out-dir utils/workbook_mlp
...
```bash
python compare_model_io_to_log_mlp.py --platform esp32 --mode replay
```

```bash
python compare_predictions_metrics_to_log_mlp.py --platform esp32 --mode replay
```

### Generate only for STM32F411RE

```bash
python compare_model_io_to_log_mlp.py --platform stm32f411re --mode replay
```

```bash
python compare_predictions_metrics_to_log_mlp.py --platform stm32f411re --mode replay
```
...
├── esp32/
│   └── replay/
│       ├── model_io_comparison_mlp_esp32_replay.xlsx
│       └── predictions_metrics_vs_log_comparison_mlp_esp32_replay.xlsx
└── stm32f411re/
    └── replay/
        ├── model_io_comparison_mlp_stm32f411re_replay.xlsx
        └── predictions_metrics_vs_log_comparison_mlp_stm32f411re_replay.xlsx
```
...
```bash
python compare_model_io_to_log_mlp.py --help
```

> The model I/O script imports `utils.global_utils.paths_mlp` during initialization. Therefore, even `--help` must be executed inside the project structure or with `RUNNER_PROJECT_ROOT` configured.
...
```bash
python compare_predictions_metrics_to_log_mlp.py --help
```
...
python compare_predictions_metrics_to_log_mlp.py \
  --platform stm32f411re \
  --mode replay \
  --pred-xlsx environment_quantized_predictions_rolling24_mlp.xlsx \
  --metrics-xlsx environment_quantized_metrics_rolling24_mlp.xlsx \
  --log device-monitor-260724-154440.zip \
...
cd C:\path\to\project
$env:RUNNER_PROJECT_ROOT = (Get-Location).Path
python .\compare_model_io_to_log_mlp.py --platform esp32
```
