"""
Script: compare_predictions_metrics_to_log_mlp.py
Module role:
...
try:
    from utils.global_utils.paths_mlp import PROJECT_ROOT, QUANTIZED_MODEL_METRICS  # type: ignore
except Exception:
    pass
...
def resolve_runtime_paths(
    ...
    if cli_pred:
        ...
        if not QUANTIZED_MODEL_METRICS:
            raise FileNotFoundError(
                "Prediction spreadsheet path not provided and utils.paths_mlp is unavailable."
            )
        ...
        pred_path = find_xlsx(
            results_dir,
            "environment_quantized_predictions_rolling24_mlp.xlsx",
            [
                "*predictions_rolling24*mlp*.xlsx",
                "*predictions*mlp*.xlsx",
                "*rolling24*mlp*.xlsx",
            ],
        )
    ...
    if cli_metrics:
        ...
        if results_dir is None:
            if not QUANTIZED_MODEL_METRICS:
                raise FileNotFoundError(
                    "Metrics spreadsheet path not provided and utils.paths_mlp is unavailable."
                )
            metrics_root = normalize_metrics_root(Path(QUANTIZED_MODEL_METRICS))
            latest_run_dir = find_latest_run_dir(metrics_root)
            results_dir = latest_run_dir / "quantization_metrics_results"
        metrics_xlsx_path = find_xlsx(
            results_dir,
            "environment_quantized_metrics_rolling24_mlp.xlsx",
            [
                "*metrics_rolling24*mlp*.xlsx",
                "*metrics*mlp*.xlsx",
                "*rolling24*mlp*.xlsx",
            ],
        )
    ...
    if cli_log:
        ...
        if cli_log_dir:
            ...
            if not PROJECT_ROOT:
                raise FileNotFoundError(
                    "Log path/directory not provided and utils.paths_mlp is unavailable."
                )
            log_dir = (
                Path(PROJECT_ROOT)
                / "firmwares"
    ...
    if cli_out:
        ...
        if cli_out_dir:
            ...
            if not PROJECT_ROOT:
                raise FileNotFoundError(
                    "Output path/directory not provided and utils.paths_mlp is unavailable."
                )
            workbook_root = Path(PROJECT_ROOT) / "utils" / "workbook_mlp"

        preferred = (
            workbook_root
            / platform_slug
            / mode_slug
            / f"predictions_metrics_vs_log_comparison_mlp_{platform_slug}_{mode_slug}.xlsx"
        )
...
def add_executive_summary_sheet(
    ...
    ws["A2"] = f"MODEL: MLP | PLATFORM: {platform_label} | MODE: {mode_slug.upper()}"
...
def main() -> None:
    ...
    parser.add_argument("--pred-xlsx", dest="pred_xlsx", help="Path to environment_quantized_predictions_rolling24_mlp.xlsx")
    ...
    parser.add_argument("--metrics-xlsx", dest="metrics_xlsx", help="Path to environment_quantized_metrics_rolling24_mlp.xlsx")
